...
router.post("/register", function(request, response) {
    ...
    if (errors.length > 0) {
        response.render("users/register", { errors, username, email, password, password2 });
    } else {
        User.findOne({ username: username }, function(error, user) {
            if (user) {
                errors.push({ msg: "Username is taken. Try another!" });
                response.render("users/register", { errors, username, email, password, password2 });
            } else {
                User.findOne({ email: email }, function(error, user) {
                    if (user) {
                        errors.push({ msg: "Email already registered. Try again!" });
                        response.render("users/register", { errors, username, email, password, password2 });
                    } else {
